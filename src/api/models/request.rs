    ...
    pub fn validate(
        ...
        if let Some(cpu) = self.cpu_time_limit_ms {
            ...
            if cpu > settings.cpu_limit_ms {
                return Err(crate::api::errors::ApiError::BadRequest(format!(
                    "cpu_time_limit_ms ({}) cannot exceed server limit of {}",
                    cpu, settings.cpu_limit_ms
                )));
            }
        }
        if let Some(mem) = self.memory_limit_mb {
            ...
            if mem > settings.memory_limit_mb {
                return Err(crate::api::errors::ApiError::BadRequest(format!(
                    "memory_limit_mb ({}) cannot exceed server limit of {}",
        ...
        if let Some(wall) = self.wall_time_limit_ms {
            ...
            if wall > settings.wall_limit_ms {
                return Err(crate::api::errors::ApiError::BadRequest(format!(
                    "wall_time_limit_ms ({}) cannot exceed server limit of {}",
                    wall, settings.wall_limit_ms
                )));
            }
        }
        Ok(())
